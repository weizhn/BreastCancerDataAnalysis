#"Ambiguous" corresponds to "mixed" in the article
...
norm_weights_path = "optimized_norm_weights_ambiguous.rds"
...
if n_columns == n_cell_types + 1:
    print("Detected extra column, likely 'Ambiguous'. Adding to cell type names.")
    cell_type_names_with_ambiguous = cell_type_names + ["Ambiguous"]
    columns_to_use = cell_type_names_with_ambiguous
...
if "Ambiguous" in weights_df.columns:
    weights_without_ambiguous = weights_df.drop("Ambiguous", axis=1)
    main_cell_types = weights_without_ambiguous.idxmax(axis=1)
    main_cell_type_probs = weights_without_ambiguous.max(axis=1)
    ambiguous_mask = weights_df["Ambiguous"] > 0.5
    if ambiguous_mask.any():
        print(f"Found {ambiguous_mask.sum()} spots with high Ambiguous probability (>0.5)")
        main_cell_types[ambiguous_mask] = "Ambiguous"
        main_cell_type_probs[ambiguous_mask] = weights_df.loc[ambiguous_mask, "Ambiguous"]
else:
...
if ("Ambiguous" %in% spots_data$cell_type) {{
  cat("Excluding Ambiguous cells from analysis...\\n")
  spots_data <- spots_data[spots_data$cell_type != "Ambiguous", ]
  cat("Number of spots after excluding Ambiguous:", nrow(spots_data), "\\n")
}}
...
try:
    ...
    if result is not None and result is not ro.rinterface.NULL:
        ...
        with open(f"{base_dir}/distance_analysis_summary_{sample}.txt", "w") as f:
            ...
            if "Ambiguous" in spots_df['cell_type'].unique():
                f.write(f"Ambiguous cells: {(spots_df['cell_type'] == 'Ambiguous').sum()}\n")
            f.write(f"Non-epithelial cells (excluding Ambiguous): {(spots_df['cell_type'] != 'Epithelial').sum()}\n\n")
