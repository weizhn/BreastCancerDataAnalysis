...
def mark_ambiguous_by_structure(df):
    df = df.copy()
    ...
    for cl in df["spatial_cluster"].unique():
        ...
        for i in range(len(sub)):
            ...
            if (
                ...
                ambiguous_idx.append(sub.index[i])

        df.loc[ambiguous_idx, "final_type"] = "Ambiguous"
...
def visualize(df, sample):
    ...
    plt.axis("equal")
    plt.title(f"{sample} - Ambiguous Filtered")
    ...
    plt.savefig(os.path.join(output_dir, f"{sample}_Ambiguous_filtered.pdf"),
                bbox_inches="tight")
    plt.close()
...
def process_sample(s):
    ...
    df2 = mark_ambiguous_by_structure(df)
    visualize(df2, s["name"])

    opt = weights_df.copy()
    amb_r = match.loc[df2["final_type"] == "Ambiguous", "r"]

    opt.iloc[amb_r] = 0
    if "Ambiguous" not in opt.columns:
        opt["Ambiguous"] = 0
    opt.loc[amb_r, "Ambiguous"] = 1

    ro.globalenv["opt"] = pandas2ri.py2rpy(opt)
    ro.r(f'saveRDS(opt, "{output_dir}/{s["name"]}_optimized_norm_weights_ambiguous.rds")')
    df2.to_csv(f"{output_dir}/{s['name']}_final_annotation.csv", index=False)

    n_total = len(df2)
    n_ambiguous = (df2["final_type"] == "Ambiguous").sum()
    print(f"{s['name']}: Total cells = {n_total}, Ambiguous cells = {n_ambiguous} ({n_ambiguous/n_total*100:.1f}%)")
